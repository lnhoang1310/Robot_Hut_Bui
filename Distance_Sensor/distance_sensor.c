...
VL53L0X_TypeDef *list_sensor[NUMS_SENSOR] = {NULL};
...
bool vl53l0x_init(VL53L0X_TypeDef *sensor, Soft_I2C_TypeDef *_i2c, GPIO_TypeDef *_xshut_port, uint16_t _xshut_pin, VL53L0X_Position _position, int8_t _offset)
{
    ...
    sensor->distance_m = 0;
    list_sensor[sensor_index++] = sensor;
...
bool vl53l0x_read_all_sensor(void)
{
    ...
    {
        if (!vl53l0x_read_range_single(list_sensor[i]))
        {
            list_sensor[i]->state = VL53L0X_ERROR;
            flag = false;
        }
    }
    return flag;
